...
def courbe_ROC(model, x_test, y_test):
    ...
    for i in range(n_classes):
        ...
        roc_data.append(
            pd.DataFrame({"FPR": fpr, "TPR": tpr, "Class": f"{class_names[i]} (AUC = {auc_score:.2f})"})
        )
...
def plot_model_curves(json_file, model_name):
    ...
    fig.add_trace(go.Scatter(x=epochs, y=history['accuracy'], mode='lines+markers', name='Training Accuracy'), row=1, col=1)
    fig.add_trace(go.Scatter(x=epochs, y=history['val_accuracy'], mode='lines+markers', name='Validation Accuracy'), row=1, col=1)
    fig.add_trace(go.Scatter(x=epochs, y=history['loss'], mode='lines+markers', name='Training Loss'), row=1, col=2)
    fig.add_trace(go.Scatter(x=epochs, y=history['val_loss'], mode='lines+markers', name='Validation Loss'), row=1, col=2)
    fig.update_layout(title=f"Model Performance Curves: {model_name}", xaxis_title="Epochs", template="plotly_white")
    fig.show()
...
def get_test_accuracy(json_file, model_name):
    ...
    if model_name in histories:
        ...
        return model_data.get('test_accuracy') or model_data.get('metrics', {}).get('test_accuracy')
...
def get_test_loss(json_file, model_name):
    ...
    if model_name in histories:
        model_data = histories[model_name]
        return model_data.get('test_loss') or model_data.get('metrics', {}).get('test_loss')
